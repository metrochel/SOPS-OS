...
int thrd_create(thrd_t *thrd, thrd_start_t start, void *args) {
    ...
    int thrd_desc = syscall2(Syscall_StartThread, (syscall_arg_t)start, (syscall_arg_t)args);
    if (!thrd_desc)
        return thrd_error;
...
thrd_t thrd_current() {
    int thrd_desc = syscall0(Syscall_GetCurrentThread);
...
int thrd_sleep(const struct timespec *duration, struct timespec *remaining) {
    ...
    int syscall_result = syscall3(Syscall_TimedBlockThread, 0, time_s, time_ns);
...
void noreturn thrd_exit(int result) {
    ...
    syscall1(Syscall_ExitThread, result);
}

int thrd_detach(thrd_t thrd) {
    return syscall1(Syscall_DetachThread, thrd.id);
}

int thrd_join(thrd_t thrd, int *result) {
    int joined_thrd_exit = syscall1(Syscall_JoinThread, thrd.id);
...
int mtx_init(mtx_t *mutex, int type) {
    ...
    int syscall_result = syscall1(Syscall_CreateMutex, type);
...
int mtx_lock(mtx_t *mtx) {
    ...
    int mtx_locked = syscall1(Syscall_GetMutexLocked, mtx->id);
    if (mtx_locked) {
        syscall1(Syscall_BlockThread, mtx->id);
        syscall1(Syscall_LockMutex, mtx->id);
        return thrd_error;
    } else {
        int result = syscall1(Syscall_LockMutex, mtx->id);
...
int mtx_timedlock(mtx_t *mtx, const struct timespec *time) {
    ...
    int mtx_locked = syscall1(Syscall_GetMutexLocked, mtx->id);
    if (mtx_locked) {
        syscall3(Syscall_BlockThread, mtx->id, dt.tv_sec, dt.tv_nsec);
        syscall1(Syscall_LockMutex, mtx->id);
        return thrd_busy;
    } else {
        int result = syscall1(Syscall_LockMutex, mtx->id);
...
int mtx_unlock(mtx_t *mtx) {
    ...
    int result = syscall1(Syscall_UnlockMutex, mtx->id);
    return result ? thrd_success : thrd_error;
}
...
void mtx_destroy(mtx_t *mtx) {
    ...
    syscall1(Syscall_DestroyMutex, mtx->id);
}
...
int cnd_init(cnd_t *cnd) {
    ...
    int syscall_result = syscall0(Syscall_CreateCondVar);
    if (!syscall_result)
        return thrd_error;
...
int cnd_signal(cnd_t *cnd) {
    ...
    int syscall_result = syscall1(Syscall_SignalCondVar, cnd->id);
...
int cnd_broadcast(cnd_t *cnd) {
    ...
    int syscall_result = syscall1(Syscall_BroadcastCondVar, cnd->id);
...
int cnd_wait(cnd_t *cnd, mtx_t *mtx) {
    ...
    int syscall_result = syscall1(Syscall_BlockThread, cnd->id);
    if (!syscall_result)
        return thrd_error;
...
int cnd_timedwait(cnd_t *cnd, mtx_t *mtx, const struct timespec *endpoint) {
    ...
    int syscall_result = syscall3(Syscall_TimedBlockThread, cnd->id, dt.tv_sec, dt.tv_nsec);
    if (!syscall_result)
        return thrd_error;
...
void cnd_destroy(cnd_t *cnd) {
    syscall1(Syscall_DestroyCondVar, cnd->id);
}
...
int tss_create(tss_t *tss, tss_dtor_t destructor) {
    ...
    int syscall_result = syscall1(Syscall_CreateTSS, (syscall_arg_t)tss->key);
    if (!syscall_result) return thrd_error;
...
void* tss_get(tss_t tss_key) {
    syscall_arg_t syscall_result = syscall1(Syscall_GetTSS, (syscall_arg_t)tss_key.key);

    return (void*)syscall_result;
}

int tss_set(tss_t tss_key, void *value) {
    int syscall_result = syscall2(Syscall_SetTSS, (syscall_arg_t)tss_key.key, (syscall_arg_t)value);

    return syscall_result ? thrd_success : thrd_error;
}

void tss_delete(tss_t tss_key) {
    syscall1(Syscall_DestroyTSS, (syscall_arg_t)tss_key.key);
}