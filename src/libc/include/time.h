...
// Значение для часового пояса, означающее время UTC
#define TIME_UTC    0

// `clock_t` - тип, способный описать количество тактов с момента запуска программы.
typedef unsigned long clock_t;

// `time_t` - тип, способный описать время в формате UNIX Timestamp (число секунд после 1 января 1970 года, 00:00:00).
...
// Если `arg` ненулевой, то записывает время в `arg`.
...
size_t strftime(char *s, size_t maxsize, const char *format, const struct tm *time);
