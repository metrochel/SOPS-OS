...
#	Полная сборка libc
#
all-libc: $(LIBC_ASM32_OBJ)
	$(ARCHIVER) rcs $(LIBC_SHARED_FILE) $(LIBC_ASM32_OBJ)
	
#
#	Установка заголовков libc в системный корень
#
install-libc: all-libc $(LIBCINCLUDE)
...
compile_kernel_cpp_source = $(CROSSCOMPILER) -c $(1) -o $(OBJSDIR_KERNEL)/$(subst .cpp,.o,$(notdir $(1))) -ffreestanding -O2 -lgc++ -Wall -Wextra -Wno-write-strings $(if $(filter-out src/kernel/graphics/text.cpp,$(1)),,-Wno-multichar) -fno-exceptions -fno-rtti $(if $(filter-out src/kernel/int/int.cpp,$(1)),,-mgeneral-regs-only) $(if $(filter-out src/kernel/acpi/sci.cpp,$(1)),,-mgeneral-regs-only)
...
$(KERNELBIN): $(KERNELOBJ) $(BUILDDIR_ETC)
	$(CROSSCOMPILER) -T $(LINKERSCRIPT) -o $(KERNELBIN) -ffreestanding -O2 -nostdlib -Xlinker -Map=$(BUILDDIR_ETC)/kernel.map $(KERNELOBJ)
